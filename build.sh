...
if [ -d "$TOOLCHAIN_DIRECTORY/$TOOLCHAIN_ARM32" ]; then
    ...
    echo -e "${txtrst}"
    git clone --depth=1 https://github.com/mvaisakh/gcc-arm.git $TOOLCHAIN_DIRECTORY/$TOOLCHAIN_ARM32
fi
...
if [ -d "$TOOLCHAIN_DIRECTORY/$TOOLCHAIN_ARM64" ]; then
    ...
    echo -e "${txtrst}"
    git clone --depth=1 https://github.com/mvaisakh/gcc-arm64.git $TOOLCHAIN_DIRECTORY/$TOOLCHAIN_ARM64
fi

#
# Build start
#
export CROSS_COMPILE=$(pwd)/$TOOLCHAIN_DIRECTORY/$TOOLCHAIN_ARM64/bin/aarch64-elf-
export CROSS_COMPILE_ARM32=$(pwd)/$TOOLCHAIN_DIRECTORY/$TOOLCHAIN_ARM32/bin/arm-eabi-
